<script>
  ...
  $effect(() => {
    if (!stateCtx.projects?.length || swiperProjects) {
      return;
    }
            ...
            >
              ...
              <div class="col-span-1 flex flex-col h-80">
                ...
                >
                  ...
                  {#if item.github_url}
                    <div class="ps-2">
                      <a href={item.github_url} target="_blank" rel="noopener noreferrer" aria-label={item.github_url}>
                        <svg
...
{:else}
  <p>{stateCtx.translation.projects?.info}</p>
{/if}
