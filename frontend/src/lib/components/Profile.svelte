...
<div class="grid grid-cols-2 gap-4">
  <div>
    ...
    <div
      class="grid grid-cols-5 py-8 gap-4 items-center [&_svg]:h-10 [&_svg]:w-10 [&_svg]:fill-gray-400 [&_svg]:hover:fill-cyan-200"
    >
    ...
    <div class="flex flex-row flex-wrap items-center gap-4 py-4">
      {#each stateCtx.skills as skill}
        {#if skill.category.name !== 'Certificate'}
          <div>
            <img src={skill.image} alt={skill.name} class="w-16 h-16 object-contain" />
          </div>
        {/if}
      {/each}
    </div>
          ...
          {#if skill.category.name === 'Certificate'}
            <div class="swiper-slide">
              ...
              >
                ...
                <div
                  class="flex flex-row gap-4 items-center text-gray-400 transition-all duration-300 group-hover:text-cyan-200 {skill.skill_url
                    ? 'cursor-pointer'
                    : ''}"
                >
                  {#if skill.skill_url}
                    ...
                    >
                      {skill.name}
                      <svg
                        class="w-6 h-6 fill-current transition-colors group-hover:fill-cyan-200"
